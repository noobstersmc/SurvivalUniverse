...
public class ChunkCommands implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("chunk")) {
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "save": {
                    //data.put(player.getUniqueId(), serializePlayerInfo(player));
                    itemStackArrayToBase64(player.getInventory().getContents());
                    break;
                }
                case "retrieve": {
                    List<String> arrays = Arrays.asList(data.get(player.getUniqueId()));
                    try {
                        player.getInventory().setContents(itemStackArrayFromBase64(arrays.get(0)));
                        player.getInventory().setArmorContents(itemStackArrayFromBase64(arrays.get(1)));
                    } catch (IOException e) {
     ...
     */
    public static String[] serializePlayerInfo(Player player) {
        JsonObject obj = new JsonObject();
        String content = toBase64(player.getInventory());
        String ender_chest = toBase64(player.getEnderChest());
        ...
        obj.add("last_known_name", new JsonPrimitive(player.getName()));
        /**
         * Serialize inventory and enderchest inventory and add them to the json object
         */
        JsonObject jsonInventory = new JsonObject();
        jsonInventory.add("held_item_slot", new JsonPrimitive(player.getInventory().getHeldItemSlot()));
        jsonInventory.add("contents", new JsonPrimitive(content));
        jsonInventory.add("armor_contents", new JsonPrimitive(armor));
        jsonInventory.add("ender_chest", new JsonPrimitive(ender_chest));
        obj.add("inventory", jsonInventory);
        /** Turn active potion effects into a json object */
        JsonObject active_effects = new JsonObject();
        player.getActivePotionEffects().stream().forEach(potion -> {
            JsonObject potionJson = new JsonObject();
            potionJson.addProperty("amplifier", potion.getAmplifier());
            potionJson.addProperty("duration", potion.getDuration());
            active_effects.add(potion.getType().getName(), potionJson);
        });
        obj.add("potion_effects", active_effects);
        /** Get health, absoption, food, saturation, level, and add them to the json */
        JsonObject player_status = new JsonObject();
        player_status.add("health", new JsonPrimitive(player.getHealth()));
        player_status.add("absorption_hearts", new JsonPrimitive(player.getAbsorptionAmount()));
        player_status.add("food_level", new JsonPrimitive(player.getFoodLevel()));
        player_status.add("saturation", new JsonPrimitive(player.getSaturation()));
        player_status.add("exp_level", new JsonPrimitive(player.getLevel()));
        player_status.add("exp_percentage", new JsonPrimitive(player.getExp()));
        obj.add("status", player_status);
        // Use this to print a pretty Gson
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        //Write JSON file
        try (FileWriter file = new FileWriter("./" + player.getUniqueId().toString()+".json")) {
